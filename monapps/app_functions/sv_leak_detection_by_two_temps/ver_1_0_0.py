...
def function(
    ...
    logger.info("'sv_leak_detection_by_two_temps_1_0_0' starts executing...")
    ...
    if end_rts > start_rts:  # all datafeed have readings with ts > cursor_ts
        ...
        temp_out_threshold = app.settings.get("temp_out_threshold", 70.0)
        temp_in_threshold = app.settings.get("temp_in_threshold", 110.0)
        ...
        for rts in grid:
            ...
            cs_ok_flag = not cs_err_flag and temp_out <= temp_out_threshold
            cs_warn_flag = not cs_err_flag and temp_out > temp_out_threshold
...
settings_jsonschema = {
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "$id": "#/settings/sv_leak_detection_by_two_temps_1_0_0",
    "title": "App settings",
    "description": "Settings for <SV leak detection by two temps 1.0.0>",
    "type": "object",
    "properties": {
        "temp_out_threshold": {"type": "number", "maximum": 100, "minimum": 50},
        "temp_in_threshold": {"type": "number", "maximum": 150, "minimum": 90},
...
sv_leak_detection_by_two_temps_1_0_0 = {
    "function": function,
    "df_schema": df_schema,
